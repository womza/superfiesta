...
function woocommerce_product_subcategories( $args = array() ) {
	...
	$product_categories_final = get_categories( apply_filters( 'woocommerce_product_subcategories_args', array(
		'parent'       => $parent_id,
		'menu_order'   => 'ASC',
		'hide_empty'   => 0,
		'hierarchical' => 1,
	...
	foreach ($terms_id as $id => $term) {
		if (isset($product_subcategories[$id])) {
			foreach ($product_subcategories[$id] as $cat) {
				array_push($product_categories, $cat);
			}
		} else {
			array_push($product_categories, $product_categories_final[$id]);
		}
	}
